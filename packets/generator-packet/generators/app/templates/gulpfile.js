...
function getGulpPacketTask(transform, format, opts = {}) {
  return getGulpTask('seeds-<%= packetName %>', transform, format, opts);
}
...
gulp.task('<%= packetName %>-scss', getGulpPacketTask('web', 'scss'));
gulp.task('<%= packetName %>-js', getGulpPacketTask('js', 'common.js'));

gulp.task('<%= packetName %>-docs', done => {
    ...
    theo.plugins.getResult(result => {
      ...
      const docsTokens = tokens.propKeys.map(key => {
        ...
        return {
          ...
          sass: sassVar(prop.packet, prop.name),
          javascript: javascriptConst(prop.packet, prop.name),
          css: cssPropertyName(prop.packet, prop.name),
          value,
          description,
          category
        };
...
gulp.task(
  'default',
  gulp.series(['clean', gulp.parallel(['<%= packetName %>-scss', '<%= packetName %>-js']), '<%= packetName %>-docs'])
);
