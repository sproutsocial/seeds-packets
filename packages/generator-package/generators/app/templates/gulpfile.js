...
gulp.task('clean', () => {
  return del(['<%= packageName %>/_generated/*']);
});
...
gulp.task('<%= packageName %>-docs', done => {
  theo.plugins.file(tokensPath).pipe(theo.plugins.transform('web')).pipe(
    theo.plugins.getResult(result => {
      ...
      makeDir('<%= packageName %>/_generated/').then(() => {
        fs.writeFileSync(
          '<%= packageName %>/_generated/<%= packageName %>.html',
          `<!-- GENERATED BY GULP - DO NOT EDIT -->\n\n<script>window.seeds<%= packageNameTitlecase %> = ${JSON.stringify(
            docsTokens
          )};</script>`
        );
        done();
